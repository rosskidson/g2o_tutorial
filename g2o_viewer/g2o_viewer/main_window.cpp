...
bool MainWindow::loadFromFile(const QString& filename)
{
  ...
  cout << "loaded " << filename.toStdString() << " with " << viewer->graph->vertices().size()
    << " vertices and " << viewer->graph->edges().size() << " measurements" << endl;

  viewer->graph->vertices();
//  int i=0;
//  for(HyperGraph::VertexIDMap::const_iterator itr = viewer->graph->vertices().begin(); itr != viewer->graph->vertices().end(); itr++)
//    cout << "v: " << i++ << ":" << itr->first << " vertex edges" << itr->second->edges().size() << "\n";
//  i = 0;
//  for(HyperGraph::EdgeSet::const_iterator itr = viewer->graph->edges().begin(); itr != viewer->graph->edges().end(); itr++)
//  {
//    cout << "e: " << i++ << ":" << (**itr).vertices().size() << "\n";
//    if(i==45)
//    {
//        HyperGraph::Vertex & v1 = *((**itr).vertices()[0]);
//        HyperGraph::Vertex & v2 = *((**itr).vertices()[1]);
//        cout << "v1: " << v1.id() << "\n";
//        cout << "v2: " << v2.id() << "\n";
//    }
//  }
